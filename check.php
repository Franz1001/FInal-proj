    ...
    <nav>
      ...
      <section id="cart-section">
          ...
          <form id="checkout-form">
              ...
              <button type="button" id="checkout-button" onclick="checkout()">Place Order</button>
          </form>
      </section>
      </div>
      <footer>
...
function checkout() {
  ...
  if (cart && cart.length > 0) {
    // Update the content of the cart section to show the order confirmation message
    const cartSection = document.getElementById('cart-section');
    cartSection.innerHTML = `
      <h2>Order Placed</h2>
      <p>Your order has been placed and is waiting for confirmation.</p>
      <p>Total: ₱<span id="cart-total">${getTotalPrice(cart).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</span></p>
    `;
  } else {
...
function displayCartItems() {
  ...
  if (cart) {
    cart.forEach((item, index) => {
      ...
      checkbox.name = 'cart-item';
      checkbox.value = index; // Use the index as the value to identify the item
